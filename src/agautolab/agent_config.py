...
def _validate_overlay(overlay: dict[str, Any]) -> None:
    ...
    for harness, facts in _table(local, "harness").items():
        ...
        if bool(facts.get("command")) == bool(facts.get("command_glob")):
            raise AgentConfigError("E_OVERLAY_SCOPE", f"local.harness.{harness} must set exactly one command key")
...
def resolve_role(config: dict[str, Any], overlay: dict[str, Any], role: str, *,
    ...
    if check_available and harness == "opencode" and provider == "ollama" and not provider_base_url:
        raise AgentConfigError("E_UNAVAILABLE", "local.provider.ollama.base_url is required by agautolab OpenCode")
...
def resolve_project_role(role: str, *, profile_override: str | None = None,
                         check_available: bool = True) -> ResolvedAgent:
    config, overlay = load_config(AGENTS_CONFIG, AGENTS_LOCAL_CONFIG)
    return resolve_role(config, overlay, role, profile_override=profile_override,
                        check_available=check_available)
