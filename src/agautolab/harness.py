"""Common process seam for every agautolab agent role."""
...
def build_argv(agent: ResolvedAgent, *, allowed_tools: str | None = None,
               add_dirs: list[str] | None = None, extra_args: list[str] | None = None,
...
def run_harness(agent: ResolvedAgent, prompt: str, *, cwd: Path, timeout: float,
                allowed_tools: str | None = None, add_dirs: list[str] | None = None,
                extra_args: list[str] | None = None, skip_permissions: bool = False,
                opencode_config: Path | None = None,
                transcript_path: Path | None = None) -> HarnessResult:
    """Launch, extract, and normalize one harness process without fallback."""
    meta = identity(agent)
    if timeout <= 0:
        return HarnessResult("agent run timed out (no budget left)", -1,
                             {**meta, "outcome": "aborted", "failure": "timeout"})
    try:
        argv = build_argv(agent, allowed_tools=allowed_tools, add_dirs=add_dirs,
    ...
    try:
        ...
        raw = (error.stdout if isinstance(error.stdout, str) else
               error.stdout.decode(errors="replace") if isinstance(error.stdout, bytes) else "")
        ...
        return HarnessResult(failure, -1, {**meta, "outcome": "aborted",
                             "duration_ms": int((time.monotonic() - started) * 1000),
                             "failure": failure}, raw)
    except OSError as error:
        failure = f"could not launch {agent.harness} ({argv[0]}): {error}"
        return HarnessResult(failure, -1, {**meta, "outcome": "failed",
                             "duration_ms": int((time.monotonic() - started) * 1000),
                             "failure": failure})
    ...
    if proc.returncode != 0:
        failure = f"{agent.harness} exited {proc.returncode}: {output.strip()[-OUTPUT_TAIL_CHARS:] or stderr_tail or 'no output'}"
    elif meta.get("is_error"):
        failure = f"{agent.harness} reported an error ({meta.get('subtype')}): {output.strip()[-OUTPUT_TAIL_CHARS:] or 'no output'}"
    ...
    return HarnessResult(output if output.strip() else (failure or ""), proc.returncode if failure is None else (proc.returncode or -1), meta, raw)
