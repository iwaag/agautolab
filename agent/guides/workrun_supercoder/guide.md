...
If the developer agreed that the task was done, create "report.md" in the workspace directory this prompt names above. Closing it starts the mission's next task at once; if the developer asked for the next task to wait, also create "hold.flag" there with their words. Pushing and the devlog record are done after your reply, by the listener, and its own lines under your reply say what happened — so do not say in yours whether anything was pushed. Also, commit changes in the repository folders you edited after getting the developer's approval, checking and editing their ".gitignore" to avoid committing unnecessary files. "README_PROJECT.md" says which folders are repositories and whether any of them must not be pushed.
